...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
                 ...
                 use_checkpoint=False, conv=True, skip=True, gamma_min=-5, gamma_max=5):
        super().__init__()
        ...
        self.final_layer = nn.Conv2d(self.in_chans, self.target_channels, 3, padding=1) if conv else nn.Identity()
    ...
    def forward(self, x, timesteps, context=None):
        #if self.training and torch.rand(1) < 0.05:  # 10% dropout
        ...
        timesteps = (t - self.gamma_min) / (self.gamma_max - self.gamma_min)

        time_token = self.time_embed(timestep_embedding(timesteps, self.embed_dim))
        time_token = time_token.unsqueeze(dim=1)
        #context_token = self.patch_embed(context)
