...
def setup_scheduler(optimizer, n_epochs, warmup_epochs=3, min_lr=0):
    if warmup_epochs > 0:
        # Only create warmup scheduler if warmup_epochs > 0
        ...
        main_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=n_epochs-warmup_epochs,  # Remaining epochs
            eta_min=min_lr  # Final learning rate
        )
        
        return torch.optim.lr_scheduler.ChainedScheduler([warmup_scheduler, main_scheduler])
    else:
        # If no warmup, just use the main scheduler with full epochs
        main_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=n_epochs,  # Use all epochs
            eta_min=min_lr  # Final learning rate
        )
